...
if __name__ == "__main__":
    ...
    # g_vs.SCREEN = pygame.display.set_mode((g_vs.WIDTH, g_vs.HEIGHT), pygame.FULLSCREEN)
    g_vs.SCREEN = pygame.display.set_mode((g_vs.WIDTH, g_vs.HEIGHT))
    head = make_transparent(pygame.image.load(g_vs.HEAD_IMAGE), g_vs.COLOR_DICT["Black"])

    window_runs = True
    while window_runs:
        g_vs.SCREEN.fill(g_vs.BACKGROUND_COLOR)
        ...
        for player in players:
            g_vs.SCREEN.blit(head, (player.pos[0] - g_vs.HEAD_RADIUS, player.pos[1] - g_vs.HEAD_RADIUS))
        pygame.display.update()
        for player in players:
            pygame.draw.circle(g_vs.SCREEN, player.color, player.pos, g_vs.THICKNESS, width=0)
